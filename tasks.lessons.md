...
## V3 Killer App Rehaul Verification — Cowork (2026-05-28 evening PT)

### `isolation: "worktree"` is broken on this Cowork mount — use sequential dispatch from main worktree
**Date:** 2026-05-28
**What happened:** Twice (this session and the prior failed one), dispatching parallel Agent tool calls with `isolation: "worktree"` returned `Failed to resolve base branch "HEAD": git rev-parse failed` for every agent in the batch. Root cause: the worktree-spawn step calls `git rev-parse HEAD`, which is a git write op that touches `.git/index.lock`. On this Cowork sandbox mount, dotfiles inside `.git/` cannot be deleted (only renamed via `mv`), so once any process leaves a stale 0-byte lock, every subsequent worktree spawn fails the same way. Even after the founder manually `rm`'d the locks from the Mac terminal, parallel agents re-created them under contention.
**Rule:** For multi-agent parallel work on the BKG repo from Cowork, do NOT use `isolation: "worktree"`. Instead: (a) dispatch one Agent at a time from the main worktree, having each `git checkout -b feature/...; do work; git checkout main`; or (b) split the work so each agent operates on completely disjoint file paths and risk file-races on shared writes. Worktree isolation that works fine on Claude Code laptop sessions silently breaks on this mount.

### Always verify merge-base vs current main before merging any feature branch older than ~24h
**Date:** 2026-05-28
**What happened:** Five V3 feature branches (`feature/v3-killerapp-rehaul-ws2..ws6`) sat on origin from a prior session, each with one commit composing a Killer App surface. Glanced at them, almost dispatched merges. Checked merge-base first: `39ba06c` — but current `main` is `07e64c8`, multiple days ahead with major work added (stage-kit, stage-shell, specialists, time-machine, Marin demo seed, auth fixes, migrations). The branches' `git diff --stat main..branch` was: `200 files changed, 649 insertions(+), 20967 deletions(-)`. Merging any one would have DELETED ~21k lines of valuable main work, not added value. The work the branches contained was real but had been superseded by direct-to-main commits from a parallel session.
**Rule:** Before merging any feature branch older than ~24h, run `git diff --stat main..branch | tail -1` and look at the deletion count. A merge that deletes >1000 lines is almost always a stale-base problem, not the developer's intent. If in doubt, rebase the branch onto current main first and re-inspect; if the rebase produces massive conflicts, the branch's work has likely already been done better on main and the branch is safely deletable.

### Bit-identical files across parallel agents confirm the "canonical schema in prompt" pattern works
**Date:** 2026-05-28
**What happened:** Inspected the 5 stale V3 WS branches that came from a parallel Cowork run on a different Mac. All 5 contained the EXACT same primitive files — confirmed via `git ls-tree` blob SHAs: `a5c2feb` for AccessibilityAdapt.tsx, `c488551` for AskAnything.tsx, etc., identical across `ws2..ws6`. This validates the strategy of embedding canonical TypeScript schemas (StanceCard, TrustStrip, etc.) verbatim in every parallel agent's prompt. The agents produced bit-for-bit identical shared files. If worktree isolation had worked, merging those branches would have deduped cleanly (no merge conflicts on the shared primitive files).
**Rule:** When dispatching parallel agents that need to share code (types, components, helpers), embed the canonical TypeScript code verbatim in every agent's prompt — do not let any agent "invent" the shared file. Determinism matters more than DRY in this case. Adds prompt length but eliminates merge conflicts and schema drift across agents.

### Doc-comments lie about composition unless verification walks the render path
**Date:** 2026-05-28
**What happened:** The V3 verification subagent caught that `alerts/page.tsx` and `rewards/page.tsx` both have header JSDoc enumerating four-lane Floor 0 questions in plain prose — but the actual render code only branches on `proMode = lane === 'professional' || lane === 'administrator'`. Public lane gets admin UI; Machine lane has no JSON payload presentation. The doc-comments were aspirational, not verified. Credentialing's same pattern (also enumerated four lanes in JSDoc) actually did branch in render and passed all 11 gates. The difference is invisible in code review unless you walk the render tree.
**Rule:** Pattern Language enumeration comments at the top of a file are a contract, not a wish. Verification passes must (a) grep for the named pieces in the JSX/component tree, AND (b) trace the rendering paths to confirm the lane/state branching actually delivers different UI for each enumerated state. A doc-only contract is worse than no contract because it generates false confidence in code review.

### When a parallel session shipped your task to main without you knowing, verify before redoing
**Date:** 2026-05-28
**What happened:** Founder pasted the V3 Killer App brief expecting me to execute it from scratch. First instinct was to start dispatching agents. Instead I ran `git diff --stat main..feature/v3-killerapp-rehaul-ws2` and checked main's `src/components/primitives/` — and discovered the entire 6-surface, 19-primitive rehaul was already shipped on main from a parallel session the founder had forgotten ran (or didn't realize had landed). Had I redispatched fresh agents, I would have either duplicated existing perfectly good code or overwritten it with something worse.
**Rule:** When the founder hands a brief that overlaps with recent main activity, do a quick `git log --since=...` and a structural inspection of the affected paths BEFORE dispatching any agents. The repo state is the ground truth, not the brief. If main has the work already, the right move is verification + cleanup + dogfood prep, not rebuild. Surface the discovery explicitly to the founder with evidence (file counts, line totals, JSDoc quotes) so they can confirm direction before destructive work.
