...
### When you write to localStorage to "remember" state, always READ from it on the rescue path
**Date:** 2026-05-06
**What happened:** Project Spine v1 wrote `bkg-active-project` to localStorage every time the user landed on `/killerapp?project=<id>`. But the workflow hooks (`useProjectWorkflowState`, `useProjectStateBlob`) only looked at `?project=` in the URL — if it was missing they bounced the user home with a `toast=needs-project`. The user clicked "Check codes" from bare `/killerapp` (no project param in URL), the workflow page redirected back home, and the user lost their context. They literally said "It should work universally after the project is described once." The localStorage value was right there, the read was missing.
**Fix:** When the hook detects a missing URL param, it now reads localStorage first. If a valid UUID is stored, it replaces the current URL with the same workflow path + the id appended — the user lands where they wanted, with context restored. Only if localStorage is empty too do we fall back to the original "bounce home with toast" path. Same pattern applied to `/killerapp` itself: when the user lands on bare `/killerapp` and we have a stored active project, redirect to `/killerapp?project=<id>` so all stage chips / action buttons see the context.
**Rule:** Whenever you write to localStorage for "active state" (active project, current tenant, selected workspace, etc.), the symmetric READ must be present on every code path that consumes that state. The most common failure is "we wrote it but only the writer knows" — every redirect, error boundary, and "missing URL param" path needs to check storage as a rescue before sending the user somewhere else. Mantra: **don't make the user re-pick what we already know.**

### A page Next classifies as `○` Static but produces no prerender HTML will 500 at runtime
**Date:** 2026-05-06
