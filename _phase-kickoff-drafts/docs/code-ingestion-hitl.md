> **DRAFT — phase-kickoff. Tier 1. For founder review. Spec only — no code executed, no schema migrated, no data mutated.**
> This file lives at `_phase-kickoff-drafts/docs/code-ingestion-hitl.md` on the `feat/shared-app-shell` worktree. Intended final path on `main`: `docs/code-ingestion-hitl.md`. The Platform Constitution's Tier 0 ingestion directive points here.

# Knowledge ingestion — the human-in-the-loop gate

*phase-kickoff draft · Tier 1 · for founder review · spec only, no code executed*
