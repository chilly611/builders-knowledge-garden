...
**↳ Google OAuth (dogfood blocker):** (1) `flowType:'implicit'` → callback `missing_code` — **FIXED + MERGED #22** (`flowType:'pkce'`); Chrome works incl. incognito. (2) Safari then failed at exchange (*"PKCE code verifier not found in storage"*) — the localStorage verifier doesn't survive Safari's cross-site redirect. **In review (PR · 2026-06-08 · fix/oauth-pkce-cookie-storage):** hybrid `auth.storage` routes the verifier to a first-party cookie (session stays in localStorage); local-verified (verifier now in a cookie, survives nav). **GATE after merge+deploy: Safari on prod** → Continue with Google → lands in app signed in. Escalation if Safari still fails: `@supabase/ssr` server-set cookies.
