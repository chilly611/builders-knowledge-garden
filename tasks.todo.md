...
### Brief 1 — "Who's asking?" voice & photo capture (M, ~1.5 weeks) — **BUILD GREEN 2026-05-12, MIGRATION PENDING**
> Status 2026-05-12: 13 files pushed (commit `0db179cc`), build red on `JSX.Element` namespace not auto-imported in client components under Next 16 + React 19. Fix pushed (commit `fa09e05a`) stripping the explicit return-type annotations from 5 files (page.tsx, WhoIsAskingClient, ContactCard, VoiceCaptureFAB, PhotoCaptureFAB) — TypeScript infers correctly. Vercel build now GREEN; route `https://builders.theknowledgegardens.com/killerapp/who-is-asking` returns HTTP 200. **MIGRATION NOT YET APPLIED** — the new tables `crm_contacts` + `crm_contact_activities` + the `crm-photos` storage bucket exist only in the file `supabase/migrations/20260512_crm_contacts.sql`. Until Chilly runs the migration, the API returns 500 ('Could not find the table public.crm_contacts'). Apply via Supabase CLI (`supabase db push`) or paste the SQL into the dashboard SQL editor (https://vercel.com/chillyd-2693s-projects/app/8P1HLzPx1MVLkqrJm2DjRX5PZR7A) so the fix can be precise. Files affected: `supabase/migrations/20260512_crm_contacts.sql`, `src/lib/crm-spine.ts`, `docs/ai-prompts/contact-extract.v1.md`, `src/app/api/v1/crm/{capture,photo,route.ts}`, `src/app/killerapp/who-is-asking/{page,WhoIsAskingClient}.tsx`, `src/components/crm/{Voice,Photo}CaptureFAB.tsx`, `src/components/crm/ContactCard.tsx`, `src/lib/__tests__/crm-spine.test.ts`, `public/llms.txt`.
