...
> **Archived 2026-06-07** when `tasks.todo.md` was reset to lead with Phase 1 — First-Paying-Contractor Path ($99/mo). The sections below were the live backlog through 2026-05-31: STAGE 2 AMENDMENT (ledger-bound chrome), ONE SHARED APP SHELL, the Owner Lane (functional + design-parity), the Lanes × Lenses data layer, the Marin Seed multi-lane cast, the Demolition + Brand-consolidation pushes, the V3 Killer App rehaul + verification, the Herbarium design-system rollout, and earlier sprints back through W9. Most are shipped or superseded; Phase 1 Lane 0 (Recon) re-verifies anything still in flight (e.g. `feat/shared-app-shell` merge state) rather than carrying stale checkboxes forward.
>
> Reference material only. The live backlog lives in `tasks.todo.md`.
...
faithful to the published standalone `public/owner-lane.html`. Reuse the shared App Shell; no schema
changes (reuse `project_change_orders` + the lanes/lens tables already in prod).

**Decisions (founder, this session):** I commit + verify; founder pushes/deploys/dogfoods. Approval
...
- [x] Demo data — single canonical Marin fixture. **Moved to `src/lib/seed-data/marin-farmhouse.ts` 2026-05-28** (the original `src/lib/demo/marin-4000.ts` is now a thin re-export shim so existing imports keep working). Constants: `MARIN_PROJECT_NAME = "Modern Farmhouse in Marin"`, `MARIN_BUDGET_TOTAL = 1_650_000`, `MARIN_BUDGET_SPENT = 312_400`, `MARIN_BUDGET_COMMITTED = 186_200`, `MARIN_BUDGET_REMAINING = 1_151_400`, `MARIN_INCOME_PROJECTED = 1_485_000` (16 draws), `MARIN_INCOME_RECEIVED = 495_000` (5 closed), `MARIN_STAGE_COMPLETION = {1:100, 2:100, 3:85, 4:42, 5..7:0}`, framing 10 wk → `computeSchedule = 37 wk`. Every page reads via `getCanonicalProject()` at `src/lib/projects/getCanonicalProject.ts`. Mock John Doe / Jane Smith team and the legacy `proj-chen-farmhouse` slug are gone (canonical UUID redirect added on `/projects/[id]`).
...
- [x] Remove AI meta footer (model · ms · LearningBadge) from AnalysisPane
- [x] Fix P0: analysis "Input for analysis" showed stale location — fixed seed + hydration guard in useProjectWorkflowState + StepCard
- [x] Fix auth pills overlapping stage chips — AuthAndProjectIndicator `inline` prop, embedded in KillerAppNav
- [x] Fix KillerAppChrome rendering behind nav (y=0) — moved inside paddingTop:48 wrapper
- [x] Remove duplicate AuthAndProjectIndicator from layout.tsx and page.tsx
- [x] Set up local .env.local via vercel env pull + Supabase dashboard
- [ ] Fix `globals.css` line 680 `:global(input)` CSS Modules syntax warning (Turbopack noise, not blocking)
