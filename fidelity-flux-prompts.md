# Killer App fidelity pass — Replicate FLUX prompt spec (DRAFT)
*Staging/research lane · canonical project = Modern Farmhouse Marin · herbarium register*
*Author: Cowork staging lane · generated, run via `stage-fidelity-assets.mjs` once creds are present*

> **Status:** locked prompts ready to run. Generate via `stage-fidelity-assets.mjs --go` once `REPLICATE_API_TOKEN` + Supabase creator creds are available, or paste into Replicate directly.
...
- **avoid (flux-dev only):** pure white, neon, fire-engine red #E8443A, oversaturated, HDR, text, watermark, logo, signage, people faces, fisheye, tilt-shift toy effect
...
Small assets for the field-log / plate strip. Square-ish. Photos: **flux-1.1-pro**, aspect_ratio: "1:1". Sketch thumb: **flux-dev**, 1:1.
...
## Run order & selection
1. Generate **both** B options (1 image each, fixed seeds) → present for **your** pick.
2. Generate C-1/2/3 as a set; if register drifts between them, re-run the odd one with the same seed family.
3. Generate T-1/2/3.
4. All land in `brand-assets` at `assets/bkg/fidelity/…` as **draft** rows. You/founder promote later.

*Seeds are fixed so the option you choose can be regenerated byte-for-similar at higher resolution for production.*
