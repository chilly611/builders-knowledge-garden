...
function MetricCard({ label, value, sub, trend }: {
  ...
  return (
    <div style={{ flex: '1 1 0', minWidth: 130, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '14px 16px' }}>
      <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '1px', margin: 0, fontWeight: 600 }}>{label}</p>
      <p style={{ fontSize: '26px', fontWeight: 800, margin: '4px 0 0', color: '#fff', letterSpacing: '-1px' }}>{value}</p>
...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <label style={{ fontSize: '11px', fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>{label}</label>
      {children}
    </div>
  );
}
...
const INPUT_STYLE: React.CSSProperties = {
  ...
  borderRadius: 8, padding: '9px 12px', color: '#fff', fontSize: '14px',
            ...
            <div style={{ padding: '0 28px 20px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              {topActions.length > 0 && (
                <div style={{ marginTop: 16, marginBottom: 16 }}>
                  <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '1px', margin: '0 0 10px' }}>
                    Today's Top Actions
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {topActions.map((action, i) => (
                      <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                        ...
                        <p style={{ fontSize: '13px', color: '#fff', margin: 0, lineHeight: 1.4 }}>{action}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Category grid */}
              <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '1px', margin: '16px 0 10px' }}>
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                        ...
                        <span style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.6)' }}>{CATEGORY_LABELS[key]}</span>
                        <span style={{ marginLeft: 'auto', width: 7, height: 7, borderRadius: '50%', background: ALERT_COLORS[c.status], flexShrink: 0 }} />
                      </div>
                      <p style={{ fontSize: '12px', fontWeight: 600, color: '#fff', margin: 0, lineHeight: 1.3 }}>{c.headline}</p>

                      <AnimatePresence>
                        {isActive && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} style={{ overflow: 'hidden' }}>
                            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', margin: '8px 0 6px', lineHeight: 1.5 }}>{c.details}</p>
                            {c.action && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 0, minHeight: 'calc(100vh - 256px)' }}>
        ...
        <div style={{ borderRight: '1px solid rgba(255,255,255,0.05)', padding: '18px 14px', overflowY: 'auto', maxHeight: 'calc(100vh - 256px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <h2 style={{ fontSize: '13px', fontWeight: 700, margin: 0, color: 'rgba(255,255,255,0.7)' }}>Needs Attention</h2>
          ...
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[1, 2, 3].map(i => <div key={i} style={{ height: 60, background: 'rgba(255,255,255,0.03)', borderRadius: 8, animation: 'pulse 2s infinite' }} />)}
            </div>
                  ...
                  return (
                    <motion.div key={item.id} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16, height: 0 }} transition={{ delay: i * 0.04 }}
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 7 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: '12px', fontWeight: 700, margin: 0, color: '#fff', lineHeight: 1.3 }}>{item.title}</p>
                          <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)', margin: '2px 0 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.project_name}</p>
                        </div>
                      </div>
                      <AnimatePresence>
                        {isExpanded && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
                            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', margin: '8px 0 0', lineHeight: 1.5, paddingLeft: 18 }}>{item.body}</p>
                            {item.options?.length > 0 && (
                              <div style={{ paddingLeft: 18, marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                                {item.options.map(opt => (
                                  <button key={opt} onClick={e => e.stopPropagation()} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '4px 9px', color: 'rgba(255,255,255,0.7)', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit' }}>{opt}</button>
                                ))}
                              </div>
                            )}
        ...
        <div style={{ padding: '18px 20px', overflowY: 'auto', maxHeight: 'calc(100vh - 256px)' }}>
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
              {[1, 2, 3].map(i => <div key={i} style={{ height: 160, background: 'rgba(255,255,255,0.03)', borderRadius: 12, animation: 'pulse 2s infinite' }} />)}
            </div>
          ) : projects.length === 0 ? (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 400, textAlign: 'center', gap: 16 }}>
              ...
              <div>
                <h3 style={{ fontSize: '20px', fontWeight: 800, margin: '0 0 8px', color: '#fff' }}>Your Command Center is ready</h3>
                <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.4)', margin: '0 0 24px', maxWidth: 340, lineHeight: 1.6 }}>Add your active projects and your AI COO will monitor them, surface risks, and keep you ahead of every deadline.</p>
                <button onClick={() => setShowAddModal(true)} style={{ background: 'linear-gradient(135deg, #E8443A, #c93328)', border: 'none', borderRadius: 10, padding: '12px 28px', color: '#fff', fontSize: '15px', fontWeight: 700, cursor: 'pointer' }}>+ Add Your First Project</button>
              </div>
            </motion.div>
          ) : (
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <h2 style={{ fontSize: '13px', fontWeight: 700, margin: 0, color: 'rgba(255,255,255,0.7)' }}>Active Projects ({projects.length})</h2>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
                {projects.map((project, i) => {
                  ...
                  return (
                    <motion.div key={project.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                      ...
                      style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${isExpanded ? `${phaseColor}30` : 'rgba(255,255,255,0.07)'}`, borderRadius: 12, padding: '14px 16px', cursor: 'pointer', transition: 'border-color 0.2s ease' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: '14px', fontWeight: 700, margin: 0, color: '#fff', lineHeight: 1.3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{project.name}</p>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                        <div>
                          ...
                          <p style={{ fontSize: '13px', fontWeight: 700, margin: '2px 0 0', color: project.budget_status === 'over' ? '#EF4444' : project.budget_status === 'ahead' ? '#22C55E' : '#fff' }}>
                            ...
                            {project.budget_amount && <span style={{ fontSize: '10px', fontWeight: 400, color: 'rgba(255,255,255,0.3)', marginLeft: 4 }}>{project.budget_status}</span>}
                          </p>
                        </div>
                        ...
                        {isExpanded && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
                            <div style={{ paddingTop: 12, marginTop: 10, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                              {project.location && <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', margin: '0 0 4px' }}>📍 {project.location}</p>}
                              {project.project_type && <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', margin: '0 0 4px' }}>🏗 {project.project_type}</p>}
                              {project.notes && <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', margin: '4px 0 0', lineHeight: 1.5 }}>{project.notes}</p>}
        ...
        {showAddModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 24, scale: 0.97 }}
              style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.6)' }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                <button onClick={() => setShowAddModal(false)} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 9, padding: '11px 0', color: 'rgba(255,255,255,0.5)', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
