...
export default function ServicesTodosClient({ workflow, stages }: Props) {
  // Project Spine v1 (Wave 3, 2026-05-06): hydrate + autosave services_todos_state.
  const {
  ...
  const handleStepComplete = (stepResult: StepResult & { workflowId: string }) => {
    // Project Spine v1: persist this step's payload into services_todos_state.
  ...
  return (
    <>
      <ProjectContextBanner project={project} selfWorkflow="services-todos" />
      <WorkflowShell
        workflow={workflow}
        stages={stages}
