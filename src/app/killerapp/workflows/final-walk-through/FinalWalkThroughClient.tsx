...
/**
 * FinalWalkThroughClient (q24, 2026-05-08 Wave 4)
 * ====================================
 *
 * The Final Walk-Through workflow. State persists in
 * command_center_projects.walk_through_state JSONB (added in
...
export default function FinalWalkThroughClient({ workflow, stages }: Props) {
  const {
  ...
  return (
    <>
      <ProjectContextBanner project={project} selfWorkflow="final-walk-through" />
      <AttachmentSection
        projectId={spineProjectId}
        workflowId="q24"
        stepId="s24-1"
        title="Take photos"
        subtitle="Walk the job and snap shots of every issue you see. The next step generates a punch list from these."
        onUploaded={(uploaded) => {
          recordStepEvent({
            type: 'step_completed',
            workflowId: 'q24',
      ...
      <WorkflowShell
        workflow={workflow}
        stages={stages}
        breadcrumbLabel="Final Walk-Through"
        contextFields={['lane']}
