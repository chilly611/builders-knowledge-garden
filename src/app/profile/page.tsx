...
const XPEngine = dynamic(() => import("@/components/XPEngine"), {
  ssr: false,
});
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{
            padding: "20px 24px", borderRadius: 14, border: "1px solid #1D9E7525",
            background: "linear-gradient(135deg, #1D9E7508, #7F77DD08)",
          }}>
            <div style={{ fontSize: 13, color: "var(--fg-secondary)", marginBottom: 16 }}>Unlock achievements by exploring dream interfaces, completing projects, and mastering construction knowledge.</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(60px, 1fr))", gap: 12 }}>
              {[
                { emoji: "🎯", name: "First Steps", desc: "Visit your first dream interface" },
                { emoji: "🏗️", name: "Builder", desc: "Complete 5 projects" },
                { emoji: "⚗️", name: "Alchemist", desc: "Transmute 10 designs" },
                { emoji: "🚀", name: "Pioneer", desc: "Explore all dream tools" },
                { emoji: "💎", name: "Master", desc: "Earn 1000+ XP" },
                { emoji: "🌟", name: "Legend", desc: "Featured in hall of fame" },
              ].map((ach, i) => (
                <div key={i} style={{
                  padding: "12px", borderRadius: 10, background: "rgba(255,255,255,0.3)",
                ...
                >
                  <div style={{ fontSize: 24, marginBottom: 4 }}>{ach.emoji}</div>
                  <div style={{ fontSize: 10, fontWeight: 600 }}>{ach.name}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Social Sharing */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Share Your Progress</div>
          <div style={{
            display: "flex", gap: 8, padding: "16px 20px", borderRadius: 14,
            ...
            {[
              ...
              <button key={social.label} style={{
                flex: 1, padding: "10px", borderRadius: 8, border: `1px solid ${social.color}30`,
                background: `${social.color}08`, color: social.color, fontSize: 12, fontWeight: 600,
                cursor: "pointer", transition: "all 0.2s",
              }}
                onMouseEnter={e => { e.currentTarget.style.background = `${social.color}15`; }}
                onMouseLeave={e => { e.currentTarget.style.background = `${social.color}08`; }}
              >
                Share
              </button>
            ))}
